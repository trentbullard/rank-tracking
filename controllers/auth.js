...
export const socialLogin = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) {
      ...
      await Log.query().insert({user_id: newUser.id, action: 'create', level: 'info', message: 'init social login', loggable_type: 'users', loggable_id: newUser.id});
      delete newUser.password_hash;
      delete newUser.session_id;
      res.json(newUser);
    } else {
...
export const signup = async (req, res, next) => {
  const data = req.body.params.data;
  try {
    const {email, passwordHash: password_hash, sessionId: session_id} = JSON.parse(decrypt(data));
    const user = await User.query().where({email}).first();
    if (!user) {
      const newUser = await User.query().insert({email, password_hash, session_id});
      await Log.query().insert({user_id: newUser.id, action: 'create', level: 'info', message: 'signup', loggable_type: 'users', loggable_id: newUser.id});
      delete newUser.password_hash;
      delete newUser.session_id;
      res.json(newUser);
    } else {
